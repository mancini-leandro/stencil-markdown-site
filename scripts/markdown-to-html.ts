...
import {
  collectHeadingMetadata,
  changeCodeCreation,
  localizeMarkdownLink
...
(async function() {
  const siteStructure = await readFile(SITE_STRUCTURE_FILE, {
    encoding: "utf8"
  });
  ...
  const filePromises = files.map(async filePath => {
    ...
    try {
      ...
      // Enable this to get Github contributors for each Markdown file
      ...
      localizeMarkdownLink(
        renderer,
        destinationFileName.replace("src", ""),
        siteStructureJson
      );
